...
def check_inspection_dates():
    ...
    try:
        ...
        for vehicle in expired_vehicles:
            logger.info(
                f"Updating vehicle {vehicle.id} inspection status from passed to pending (expired)"
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to send expired notification for vehicle {vehicle.id}: {str(e)}"
                )
        ...
        for vehicle in upcoming_vehicles:
            ...
            logger.info(
                f"Vehicle {vehicle.id} inspection expires in {days_remaining} days"
            )

            try:
                NotificationService.send_inspection_reminder_notification(
                    db, vehicle, days_remaining
                )
            except Exception as e:
                logger.error(
                    f"Failed to send reminder notification for vehicle {vehicle.id}: {str(e)}"
                )

        # Commit all changes
        db.commit()
        logger.info(
            f"Updated {len(expired_vehicles)} expired vehicles and notified about {len(upcoming_vehicles)} upcoming inspections"
        )
