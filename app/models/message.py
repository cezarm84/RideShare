...
class Conversation(Base):
    ...
    ride_id = Column(
        Integer, ForeignKey("rides.id"), nullable=True
    )  # Optional link to a ride
...
class Message(Base):
    ...
    recipient = relationship(
        "User", foreign_keys=[recipient_id], overlaps="received_messages"
    )
    ...
    message_type = Column(
        String, default="text"
    )  # text, location, image, file, audio, video
    message_metadata = Column(
        Text, nullable=True
    )  # JSON data for rich messages (locations, etc.)
