...
class Hub(Base):
    ...
    __tablename__ = "hubs"
    __table_args__ = {'extend_existing': True}  # Add this line to fix table redefinition errors
    ...
    starting_rides = relationship("Ride", foreign_keys="Ride.starting_hub_id", back_populates="starting_hub")
    destination_rides = relationship("Ride", foreign_keys="Ride.destination_hub_id", back_populates="destination_hub")
...
class HubPair(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    source_hub_id = Column(Integer, ForeignKey("hubs.id", ondelete="CASCADE"), nullable=False)
    destination_hub_id = Column(Integer, ForeignKey("hubs.id", ondelete="CASCADE"), nullable=False)
    expected_travel_time = Column(Integer, nullable=True, comment="Expected travel time in minutes")
