...
async def get_attachment_statistics(
    period: str = Query("all", description="Time period for statistics: 'day', 'week', 'month', 'year', or 'all'"),
    user_specific: bool = Query(False, description="Whether to get statistics only for the current user's attachments"),
    ...
    if user_specific:
        ...
        query = query.join(Message, MessageAttachment.message_id == Message.id) \
    ...
    if period != "all":
        file_type_counts = file_type_counts.filter(MessageAttachment.created_at >= start_time)
    ...
    if period != "all":
        total_size_query = total_size_query.filter(MessageAttachment.created_at >= start_time)
    ...
    if not user_specific:
        ...
        if period != "all":
            top_uploaders_query = top_uploaders_query.filter(MessageAttachment.created_at >= start_time)
        ...
        "file_types": [
            {
                ...
                "count": ftype.count,
                "percentage": round((ftype.count / total_count * 100), 2) if total_count > 0 else 0
            } 
...
async def get_user_storage_info(
    ...
    total_size = db.query(func.sum(MessageAttachment.file_size)) \
    ...
    for i in range(6):
        ...
        month_size = db.query(func.sum(MessageAttachment.file_size)) \
                      .join(Message, MessageAttachment.message_id == Message.id) \
                      .filter(Message.sender_id == current_user.id) \
    ...
    return {
        ...
        "percentage_used": round((total_size / max_storage * 100), 2) if max_storage > 0 else 0,
        ...
        "storage_by_type": [
            {
                ...
                "percentage": round((storage_type.total_size / total_size * 100), 2) if total_size > 0 else 0
            }
