...
def get_conversation(
    ...
    """
    conversation = MessageService.get_conversation(db=db, conversation_id=conversation_id)
    if not conversation:
        raise HTTPException(
...
@router.post("/conversations/{conversation_id}/participants/{user_id}", response_model=ConversationInDB)
def add_participant(
    conversation_id: int = Path(..., gt=0),
...
@router.delete("/conversations/{conversation_id}/participants/{user_id}", response_model=ConversationInDB)
...
def create_message(
    ...
    conversation = MessageService.get_conversation(db=db, conversation_id=message_in.conversation_id)
    if not conversation:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Conversation not found"
...
def update_message_settings(
    ...
    settings = MessageService.update_user_message_settings(
        db=db, 
        user_id=current_user.id, 
        obj_in=settings_in.dict(exclude_unset=True)
    )
    return settings
