...
def create_route(
    ...
    if route_data.starting_hub_id:
        starting_hub = db.query(Hub).filter(Hub.id == route_data.starting_hub_id).first()
        if not starting_hub:
            raise HTTPException(
    ...
    if route_data.destination_hub_id:
        destination_hub = db.query(Hub).filter(Hub.id == route_data.destination_hub_id).first()
