...
from app.api.dependencies import get_current_user, get_db, websocket_auth
...
async def create_direct_conversation(
    ...
    try:
        conversation = messaging_service.create_or_get_direct_conversation(
            current_user.id, data.other_user_id
        )
        return conversation
    except HTTPException:
        raise
...
async def create_ride_conversation(
    ...
    try:
        conversation = messaging_service.create_ride_conversation(
...
async def send_message(
    ...
    try:
        ...
        message = await messaging_service.send_message(
            current_user.id, conversation_id, message_data
        )
        return message
    except HTTPException:
        raise
        ...
        while True:
            ...
            try:
                ...
                if message_type == "ping":
                    ...
                    conversation_id = message_data.get("conversation_id")
                    if conversation_id:
                        connection_manager.unregister_conversation(
                            user.id, conversation_id
                        )
                        await websocket.send_text(
                            json.dumps(
                    ...
                    if conversation_id and content:
                        ...
                        sent_message = await messaging_service.send_message(
                            user.id, conversation_id, msg_create
                        )

                        # Confirm to sender
                        await websocket.send_text(
                            json.dumps(
                                {"type": "message_sent", "message": sent_message}
                            )
                        )
