...
from app.schemas.matching_preference import UserMatchingPreferenceCreate, UserMatchingPreferenceResponse, UserMatchingPreferenceUpdate
import logging
...
async def get_matching_preferences(
    ...
    """
    preferences = db.query(UserMatchingPreference).filter(
        UserMatchingPreference.user_id == current_user.id
    ).first()
...
async def update_matching_preferences(
    ...
    """
    existing_preferences = db.query(UserMatchingPreference).filter(
        UserMatchingPreference.user_id == current_user.id
    ).first()
