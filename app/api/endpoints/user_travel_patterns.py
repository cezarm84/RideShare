...
async def get_user_travel_patterns(
    ...
    """
    patterns = db.query(UserTravelPattern).filter(
        UserTravelPattern.user_id == current_user.id
    ).all()
    ...
    for pattern in patterns:
        ...
        if pattern.destination_type == "hub" and pattern.destination_id:
            ...
            pattern_dict["destination_name"] = f"Custom Location ({pattern.destination_latitude:.4f}, {pattern.destination_longitude:.4f})"
