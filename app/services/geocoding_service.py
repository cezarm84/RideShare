...
class GeocodingService:
    ...
    def batch_geocode(self, addresses: List[Address], db: Session) -> int:
        ...
        try:
            ...
            db.commit()
            logger.info(f"Successfully geocoded {success_count} out of {len(addresses)} addresses")
        except Exception as e:
            db.rollback()
    ...
    def reverse_geocode(self, latitude: float, longitude: float) -> Optional[dict]:
        ...
        try:
            ...
            if self.db:
                ...
                if cached_address:
                    logger.info(f"Retrieved address for coordinates from cache: {latitude}, {longitude}")
            ...
            if 'address' in data:
                # Save to cache if database session is available
                if self.db:
                    self._save_reverse_to_cache(f"{latitude},{longitude}", data['address'])
                return data['address']
            else:
    ...
    def _save_to_cache(self, address: str, coordinates: Tuple[float, float]) -> None:
        ...
        try:
            ...
            existing = self.db.query(GeocodingCache).filter(
                GeocodingCache.address == address,
                GeocodingCache.cache_type == 'forward'
            ).first()
