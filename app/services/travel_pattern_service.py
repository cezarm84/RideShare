...
class TravelPatternService:
    ...
    def extract_patterns_from_bookings(self, user_id: int) -> list:
        ...
        bookings = (
            ...
            .filter(RideBooking.user_id == user_id, RideBooking.status == "completed")
            .all()
        )
        ...
        for booking in bookings:
            ...
            if (
                route_patterns[route_key]["time_slots"][time_key]["last_traveled"]
                is None
                or ride_date
                > route_patterns[route_key]["time_slots"][time_key]["last_traveled"]
            ):
                ...
                patterns.append(
                    {
                        ...
                        "departure_time": datetime.strptime(
                            time_data["departure_time"], "%H:%M:%S"
                        ).time(),
    ...
    def update_user_travel_patterns(self, user_id: int) -> int:
        ...
        existing_patterns = (
            ...
            .filter(UserTravelPattern.user_id == user_id)
            .all()
        )
    ...
    def update_pattern_after_ride(self, user_id: int, ride_id: int) -> bool:
        ...
        if not booking:
            logger.warning(
                f"No completed booking found for user {user_id} and ride {ride_id}"
            )
            return False
        ...
        existing_pattern = (
            ...
            .filter(
                ...
                func.abs(
                    func.extract("hour", UserTravelPattern.departure_time)
                    - func.extract("hour", departure_time)
                )
                < 1,
            )
            .first()
