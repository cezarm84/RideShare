...
class FileService:
    ...
    def save_file(file: UploadFile, user_id: int) -> Dict[str, Any]:
        ...
        result = {
            ...
            "file_type": file_type,
            "upload_path": os.path.join("uploads", str(user_id), year_month, unique_filename),
        ...
        if file_type in FileService.ALLOWED_IMAGE_TYPES:
            try:
                with Image.open(file.file) as img:
                    ...
                    thumbnail_dir = os.path.join(base_dir, str(user_id), year_month, "thumbnails")
                    ...
                    result["thumbnail_url"] = f"{settings.API_BASE_URL}/media/{os.path.join('uploads', str(user_id), year_month, 'thumbnails', unique_filename)}"
    ...
    @staticmethod
    def create_message_attachment(db: Session, message_id: int, file_metadata: Dict[str, Any]) -> MessageAttachment:
