...
class ContactService:
    ...
    def create_message(self, obj_in: ContactMessageCreate, user_id: Optional[int] = None) -> ContactMessage:
    ...
    def get_messages_by_email(self, email: str, skip: int = 0, limit: int = 100) -> List[ContactMessage]:
        """Get all contact messages for a specific email."""
        return self.get_messages(skip=skip, limit=limit, email=email)

    def get_messages_by_user(self, user_id: int, skip: int = 0, limit: int = 100) -> List[ContactMessage]:
