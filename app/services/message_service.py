...
from app.models.message import Message, Conversation, UserMessageSettings, conversation_participants
from app.models.user import User
from app.schemas.message import MessageCreate, MessageUpdate, ConversationCreate, ConversationUpdate

class MessageService:
    """Service for handling message and conversation operations"""
    
    @staticmethod
    def create_conversation(db: Session, obj_in: ConversationCreate, creator_id: int) -> Conversation:
    ...
    @staticmethod
    def get_conversations_for_user(db: Session, user_id: int, skip: int = 0, limit: int = 100) -> List[Conversation]:
    ...
    @staticmethod
    def update_conversation(db: Session, conversation_id: int, obj_in: ConversationUpdate) -> Optional[Conversation]:
    ...
    @staticmethod
    def add_participant(db: Session, conversation_id: int, user_id: int) -> Optional[Conversation]:
    ...
    @staticmethod
    def remove_participant(db: Session, conversation_id: int, user_id: int) -> Optional[Conversation]:
    ...
    @staticmethod
    def get_messages(db: Session, conversation_id: int, skip: int = 0, limit: int = 50) -> List[Message]:
    ...
    def get_unread_messages_count(db: Session, user_id: int) -> int:
        ...
        user_conversations = (db.query(Conversation.id)
                             .join(conversation_participants)
                             .filter(conversation_participants.c.user_id == user_id)
                             .filter(Conversation.is_active == True))
    ...
    @staticmethod
    def get_user_message_settings(db: Session, user_id: int) -> Optional[UserMessageSettings]:
        """Get message settings for a user"""
        settings = db.query(UserMessageSettings).filter(UserMessageSettings.user_id == user_id).first()
    ...
    @staticmethod
    def update_user_message_settings(db: Session, user_id: int, obj_in: Dict[str, Any]) -> Optional[UserMessageSettings]:
