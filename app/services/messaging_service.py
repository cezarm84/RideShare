...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: int):
        ...
        logger.info(f"User {user_id} connected to messaging. Active connections: {len(self.active_connections)}")
    ...
    def disconnect(self, websocket: WebSocket, user_id: int):
        ...
        if user_id in self.active_connections:
            ...
            logger.info(f"User {user_id} disconnected from messaging. Remaining connections: {len(self.active_connections)}")
    ...
    async def send_conversation_message(self, message: Dict[str, Any], conversation_id: int, exclude_user_id: Optional[int] = None):
...
class MessagingService:
    ...
    def get_user_conversations(self, user_id: int, skip: int = 0, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        for conversation in conversations:
            ...
            other_participants = [
                {
                    ...
                    "is_driver": True if conversation.ride and conversation.ride.driver_id == p.id else False
                }
            ...
            conv_info = {
                "id": conversation.id,
                "title": conversation.title or self._generate_conversation_title(conversation, user_id),
                ...
                "last_message": {
                    ...
                    "is_system_message": latest_message.is_system_message if latest_message else False
    ...
    def get_conversation_messages(self, conversation_id: int, user_id: int, skip: int = 0, limit: int = 50) -> List[Dict[str, Any]]:
        """
        ...
        for message in messages:
            ...
            msg_data = {
                ...
                "sender": {
                    ...
                    "is_driver": True if conversation.ride and conversation.ride.driver_id == sender.id else False
                } if sender else None
            }
    ...
    async def send_message(self, user_id: int, conversation_id: int, message_data: MessageCreate) -> Dict[str, Any]:
        """
        ...
        if not conversation:
            raise HTTPException(status_code=403, detail="User cannot send messages to this conversation")
    ...
    def create_conversation(self, creator_id: int, data: ConversationCreate) -> Dict[str, Any]:
        """
        ...
        if len(existing_user_ids) != len(all_participant_ids):
            ...
            raise HTTPException(status_code=400, detail=f"Some users do not exist: {missing}")
        ...
        if data.ride_id:
            ...
            if existing_conv:
                ...
                if missing_participants:
                    missing_users = self.db.query(User).filter(User.id.in_(missing_participants)).all()
        ...
        participants = self.db.query(User).filter(User.id.in_(all_participant_ids)).all()
    ...
    def create_or_get_direct_conversation(self, user_id: int, other_user_id: int) -> Dict[str, Any]:
        """
        ...
        participants = self.db.query(User).filter(
    ...
    async def send_system_message(self, conversation_id: int, content: str) -> Dict[str, Any]:
        """
    ...
    def _format_conversation(self, conversation: Conversation, user_id: int) -> Dict[str, Any]:
    ...
    def _generate_conversation_title(self, conversation: Conversation, user_id: int) -> str:
        ...
        if conversation.conversation_type == "direct":
            other_participant = next((p for p in conversation.participants if p.id != user_id), None)
            if other_participant:
                return f"{other_participant.first_name} {other_participant.last_name}"
        
        # Fallback for group chats
        participant_names = [f"{p.first_name}" for p in conversation.participants if p.id != user_id]
