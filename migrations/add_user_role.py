...
def run_migration():
    ...
    try:
        ...
        if "role" not in column_names:
            ...
            cursor.execute("UPDATE users SET role = 'superadmin' WHERE is_superadmin = 1")
            
            # Set driver roles
            cursor.execute("UPDATE users SET role = 'driver' WHERE user_type = 'driver'")
            
            # Set manager roles for business users (example rule)
            cursor.execute("UPDATE users SET role = 'manager' WHERE user_type = 'business'")
