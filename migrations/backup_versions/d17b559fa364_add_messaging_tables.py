...
def upgrade() -> None:
    ...
    if not table_exists('geocoding_cache'):
        ...
        op.create_index(op.f('ix_geocoding_cache_address'), 'geocoding_cache', ['address'], unique=True)
        op.create_index(op.f('ix_geocoding_cache_id'), 'geocoding_cache', ['id'], unique=False)
    
    # Create user_message_settings table if it doesn't exist
    if not table_exists('user_message_settings'):
        op.create_table('user_message_settings',
