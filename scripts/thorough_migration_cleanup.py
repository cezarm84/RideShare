...
def thorough_migration_cleanup():
    ...
    try:
        ...
        for filename in os.listdir(versions_dir):
            if filename.endswith('.py'):
                with open(os.path.join(versions_dir, filename), 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()
                    # Check if this file contains the duplicate revision ID
                    if "revision: str = '2f8c936ae712'" in content or "revision = '2f8c936ae712'" in content:
        ...
        if os.path.exists(vehicle_type_file):
            ...
            print(f"Moved add_vehicle_type_to_rides.py to backup")
