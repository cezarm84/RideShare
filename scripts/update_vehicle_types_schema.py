...
def update_vehicle_types_schema():
    ...
    try:
        ...
        if not check_column_exists(db, "vehicle_types", "min_capacity"):
            ...
            db.execute(text("ALTER TABLE vehicle_types ADD COLUMN min_capacity INTEGER DEFAULT 1"))
            
            # Update existing vehicle types with appropriate min capacities
            db.execute(text("UPDATE vehicle_types SET min_capacity = 1 WHERE name = 'car'"))
            db.execute(text("UPDATE vehicle_types SET min_capacity = 5 WHERE name = 'mini_van'"))
            db.execute(text("UPDATE vehicle_types SET min_capacity = 9 WHERE name = 'bus'"))
        ...
        if not check_column_exists(db, "vehicle_types", "max_capacity"):
            ...
            db.execute(text("ALTER TABLE vehicle_types ADD COLUMN max_capacity INTEGER DEFAULT 50"))
            
            # Update existing vehicle types with appropriate max capacities
            db.execute(text("UPDATE vehicle_types SET max_capacity = 4 WHERE name = 'car'"))
            db.execute(text("UPDATE vehicle_types SET max_capacity = 8 WHERE name = 'mini_van'"))
            db.execute(text("UPDATE vehicle_types SET max_capacity = 50 WHERE name = 'bus'"))
        ...
        for vt in vehicle_types:
            existing = db.execute(text(f"SELECT id FROM vehicle_types WHERE name = '{vt['name']}'")).fetchone()
            if not existing:
