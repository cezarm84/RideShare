...
def fix_ride_destination(ride_id=2):
    ...
    try:
        ...
        starting_hub = db.execute(f"SELECT * FROM hubs WHERE id = {ride.starting_hub_id}").fetchone()
        if not starting_hub:
        ...
        hubs = db.execute("SELECT * FROM hubs WHERE id != :starting_hub_id", 
                         {"starting_hub_id": ride.starting_hub_id}).fetchall()
        ...
        if not hubs:
            ...
            new_hub = db.execute("SELECT * FROM hubs ORDER BY id DESC LIMIT 1").fetchone()
        ...
        db.commit()
        print(f"Successfully updated ride {ride_id} with destination hub ID {destination_hub_id}")
        
        # Verify the update
        updated_ride = db.execute(f"SELECT * FROM rides WHERE id = {ride_id}").fetchone()
