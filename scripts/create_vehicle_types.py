...
def create_standard_vehicle_types():
    ...
    try:
        ...
        try:
            ...
            if existing_types:
                logger.info(f"Found {len(existing_types)} existing vehicle types. No need to create defaults.")
                for vtype in existing_types:
                    logger.info(f"  - {vtype.name}: capacity {vtype.capacity}, price factor {vtype.price_factor}")
                return
