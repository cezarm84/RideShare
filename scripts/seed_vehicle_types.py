...
def create_vehicle_types():
    ...
    try:
        ...
        try:
            ...
            logger.info("Make sure the table exists - you may need to run migrations first.")
            return
        ...
        for vt in vehicle_types:
            ...
            existing = db.query(VehicleType).filter(VehicleType.name == vt["name"]).first()
            
            if existing:
                logger.info(f"Vehicle type '{vt['name']}' already exists with ID {existing.id}")
                ...
                db.add(vehicle_type)
                created_count += 1
                logger.info(f"Added new vehicle type: {vt['name']} (capacity: {vt['min_capacity']}-{vt['max_capacity']})")
